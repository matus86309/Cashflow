...
const Expenses: React.FC = () => {
  ...
  const getChartData = () => {
    ...
    if (chart === 1) {
      ...
      return [
        months.map((m) => {
          if (m.expense.length > 0) {
            const balance = m.expense
      ...
      return [{ name: "Uncategorised" }, ...categories.expense].map((cat) => {
        return months.map((m) => {
          let y = 0;
          let noData = true;
  ...
  const getLogs = (): Log[] => {
    return getLogsFromPeriod(DBLogs, { dateFrom, dateTo }, "expense");
  };

  const getCategories = () => {
    const logs = getLogs();
    if (categories.expense.length > 0)
      return [{ name: "Uncategorised" }, ...categories.expense].map((c) => {
        let balance = 0;
                ...
                <IonRow>
                  <CustomCardHeader>
                    <div className="header-select-wrapper">
                      ...
                      >
                        <IonSelectOption value={1}>Expenses</IonSelectOption>
            ...
            {showDT && (
              <IonRow>
                <IonCol size="6" className="date">
                  From:
                  <IonDatetimeButton datetime="ef"></IonDatetimeButton>
                ...
                <IonCol size="6" className="date">
                  To:
                  <IonDatetimeButton datetime="et"></IonDatetimeButton>
                  <IonModal keepContentsMounted>
                    <IonDatetime
                      id="et"
                      presentation="date"
